...
import { deleteUserCompletedRecipe } from '../../src/api/CompletedRecipes';
...
describe('deleteUserCompletedRecipe', () => {
  it('should delete completed recipe for a user', async () => {
    const req: any = {
      body: {
    ...
    await deleteUserCompletedRecipe(req, res);

    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith({
      error: false,
      data: {},
      message: 'Successfully removed completed recipe'
    });
  });
  ...
  it('should handle case when there is an internal server error', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({
      error: true,
      data: null,
      message: 'Could not remove completed recipe'
    });
  });
});
